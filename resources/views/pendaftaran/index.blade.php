<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if (Auth::user() && in_array(Auth::user()->roles, ['Pekerja']))
                <a href="{{ route('pendaftaran.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Tambah
                    Pendaftaran</a>
            ...
            <table class="w-full border mt-4">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        @if (Auth::user() && in_array(Auth::user()->roles, ['Admin', 'Perekrut']))
                            <th class="border px-4 py-2">Calon Pekerja</th>
                        @endif
                        ...
                        <th class="border px-4 py-2">Pengalaman</th> <!-- Menambahkan Deskripsi -->
                        <th class="border px-4 py-2">Keahlian</th> <!-- Menambahkan Kategori -->
                ...
                <tbody>
                    @if ($pendaftaran->count() > 0)
                        @foreach ($pendaftaran as $index => $daftar)
                            <tr>
                                ...
                                @if (Auth::user() && in_array(Auth::user()->roles, ['Admin', 'Perekrut']))
                                    <td class="border px-4 py-2">{{ $daftar->user->name ?? 'Tidak Diketahui' }}</td>
                                @endif
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                        <div class="flex justify-center gap-2">
                                            <form action="{{ route('pendaftaran.terima', $daftar->id) }}"
                                                method="POST" onsubmit="return confirm('Terima pendaftaran ini?');">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit"
                                            ...
                                            <form action="{{ route('pendaftaran.tolak', $daftar->id) }}" method="POST"
                    ...
                    @else
                        <tr>
                            <td colspan="8" class="border text-center px-4 py-2">Tidak ada data pendaftaran</td>
                        </tr>
                    @endif
